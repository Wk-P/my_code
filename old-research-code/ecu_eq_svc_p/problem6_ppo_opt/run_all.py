...
def plot_training_curve(cb: P6Callback, ilp_ar: float, outdir: Path, scenario_name: str):
    ...
    np.savez(outdir / "train_data.npz", timesteps=ts, episode_ars=np.array(cb.episode_ars), cap_viol_rates=np.array(cb.episode_cap_violations, dtype=float)/C.M, conf_viol_rates=np.array(cb.episode_conflict_violations, dtype=float)/C.M)
