...
def plot_training_curve(cb: P3Callback, ilp_ar: float, outdir: Path, scenario_name: str):
    ...
    np.savez(outdir / "train_data.npz", timesteps=ts, episode_ars=np.array(cb.episode_ars), cap_viol_rates=np.array(cb.episode_cap_viol_rates), conf_viol_rates=np.array(cb.episode_conf_viol_rates))
