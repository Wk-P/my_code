"""run_all.py - One-shot DDQN full pipeline:
  ...
  3. Train DDQN (no action masking)     -> training curve
  4. Evaluate trained DDQN              -> ddqn_ars + violations
  5. Produce plots:
    - comparison.png     - AR box plot + violation rate bar (2-way: ILP vs DDQN)
...
def plot_training_curve(cb, ilp_ar, outdir, scenario_name):
    ...
    np.savez(outdir / "train_data.npz", timesteps=ts, episode_ars=np.array(cb.episode_ars), cap_viol_rates=np.array(cb.episode_cap_violated, dtype=float), conf_viol_rates=np.array(cb.episode_conf_violated, dtype=float))
...
def main():
    ...
    # 3. DDQN training
    print(f"\n[2/3] DDQN training ({C.TOTAL_STEPS:,} steps) ...")
    ...
    # 4. DDQN evaluation
    print(f"\n[3/3] DDQN evaluation ({len(C.TEST_SCENARIOS)} episodes, deterministic) ...")
    def ddqn_policy(obs):
        action, _ = model.predict(obs, deterministic=True)
        return int(action)
    ddqn_res = run_episodes(ecus, services, ddqn_policy)
