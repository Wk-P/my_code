...
class LagrangeEnv(gym.Env):
    ...
    Observation (shape: 5N+7+M):
        ...
        [6+5N+M]     current λ value, normalised by λ_max
    ...
    def __init__(self, ecus: list[ECU], services: list[SVC],
        ...
        self.observation_space = gym.spaces.Box(
            low=-1.0, high=1.0, shape=(5 * self.N + 7 + self.M,), dtype=np.float32,
        )
    ...
    def _build_conflict_map(self) -> dict:
        # svc_idx -> list of frozensets that contain it (for fast lookup)
        m = {}
        for subset in self.conflict_sets:
            fs = frozenset(subset)
            for s in subset:
                m.setdefault(s, []).append(fs)
        return m

    def _update_ecu_allowed(self, ecu_idx: int, svc_idx: int) -> None:
        for fs in self._conflict_map.get(svc_idx, []):
            self.ecu_allowed[ecu_idx] -= (fs - {svc_idx})
    ...
    def reset(self, seed=None, options=None):
        ...
        # cache episode-invariant values
        self._max_cap   = float(np.max(self.initial_vms) + 1e-8)
        self._total_cap = float(np.sum(self.initial_vms) + 1e-8)
        self._initial_cap_pct = self.initial_vms / self._max_cap
        # running sum and array for remaining service demands
        reqs = np.array([s.requirement for s in self.services], dtype=np.float32)
        self._remaining_demand = float(reqs.sum())
        self._remaining_svcs   = reqs / self._max_cap   # will zero out as steps advance
        # precompute conflict map
        self._conflict_map = self._build_conflict_map()
        return self._obs(), {}
    ...
    def _obs(self) -> np.ndarray:
        ...
        if self._step >= self.M:
            ...
            svc = self.services[self._step]
            service_demand_norm = np.float32(svc.requirement / mc)
            ...
            remaining_service_demand_sum = np.float32(self._remaining_demand / tc)
            remaining_services_count     = np.float32((self.M - self._step) / self.M)
            remaining_usable_ecu_count   = np.float32(
                np.sum(self.remaining_vms >= svc.requirement) / self.N
            )

        remaining_abs_norm = np.clip(self.remaining_vms, -mc, mc) / mc
        remaining_usable_capacity_sum = np.float32(
            np.sum(np.clip(self.remaining_vms, 0.0, None)) / tc
        )
        ...
        return np.concatenate([
            ...
            self._initial_cap_pct,
            remaining_abs_norm,
            conflict_flag,
            ecu_allowed_frac,
            valid_flag,
            self._remaining_svcs,
    ...
    def step(self, action: int):
        ...
        if not self.ecu_placements[action]:
        ...
        self.ar = self._total_ru / self._active_count if self._active_count > 0 else 0.0
        self._remaining_demand -= svc.requirement
        self._remaining_svcs[self._step] = 0.0
        self._step += 1

        done = self._step >= self.M
        terminal_bonus = 0.0
        if done:
            terminal_bonus = self.ar if self.episode_violations == 0 else -1.0
...
if __name__ == "__main__":
    ...
    obs, _ = env.reset()
    print(f"Obs shape: {obs.shape}  (expected {5*N + 7 + M})")
