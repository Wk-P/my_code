...
class LagrangeState:
    """Shared dual-ascent multiplier for constraint penalties.

    lambda += lr * (observed_violation_rate - target_rate), clipped to
    [lambda_min, lambda_max]. Target is 0 (we want zero violations), so the
    multiplier only grows while violations persist and relaxes once the
    policy stops violating. Pass one instance to every parallel env so all
    workers read/write the same multipliers.
    """

    def __init__(self, lr: float = 0.05, lambda_init: float = 2.0,
                 lambda_min: float = 0.0, lambda_max: float = 50.0):
        self.lr = lr
        self.lambda_min = lambda_min
        self.lambda_max = lambda_max
        self.lambda_cap = lambda_init
        self.lambda_conflict = lambda_init

    def update(self, cap_rate: float, conflict_rate: float) -> None:
        self.lambda_cap = float(np.clip(
            self.lambda_cap + self.lr * cap_rate, self.lambda_min, self.lambda_max))
        self.lambda_conflict = float(np.clip(
            self.lambda_conflict + self.lr * conflict_rate, self.lambda_min, self.lambda_max))
...
class DQNEnv(gym.Env):
    ...
    constraint_mode (added for LT-deviation investigation, see docs_cn.md):
        "soft"     (default) — as documented above, episode always runs M steps.
        "hard"     — either violation type terminates the episode immediately
                     with a -(unplaced services) penalty, mirroring the EQ/GT
                     envs' hard-termination design so a single episode cannot
                     accumulate many violations.
        "lagrange" — replaces the fixed -2.0 penalties with -lambda_cap /
                     -lambda_conflict, where the multipliers live in a shared
                     LagrangeState updated via dual ascent towards zero
                     violations (mirrors problem5_ppo_lagrangian).
    """

    metadata = {"render_modes": []}

    def __init__(self, ecus: list[ECU], services: list[SVC], scenarios=None,
                 constraint_mode: str = "soft", lagrange_state=None,
                 cap_penalty_weight: float = 2.0, conflict_penalty_weight: float = 2.0):
        super().__init__()
        assert constraint_mode in ("soft", "hard", "lagrange")
        self._scenarios = scenarios
        self.constraint_mode = constraint_mode
        self.lagrange_state = lagrange_state
        # Fixed penalty weights used by "soft"/"hard" modes (ignored by "lagrange",
        # which reads lambda_cap/lambda_conflict from lagrange_state instead).
        # Conflict defaults equal to capacity, but can be set higher — empirically,
        # DQN/DDQN learn to avoid capacity overflow much faster than conflicts
        # under an equal -2.0/-2.0 split, so conflict_penalty_weight > cap_penalty_weight
        # lets us test whether a stronger signal closes that gap.
    ...
    def step(self, action: int):
        ...
        if self.constraint_mode == "lagrange":
            lam_cap  = self.lagrange_state.lambda_cap      if self.lagrange_state else self.cap_penalty_weight
            lam_conf = self.lagrange_state.lambda_conflict if self.lagrange_state else self.conflict_penalty_weight
        else:
            lam_cap, lam_conf = self.cap_penalty_weight, self.conflict_penalty_weight
        cap_penalty      = -lam_cap  if cap_violated      else 0.0
        conflict_penalty = -lam_conf if conflict_violated else 0.0
        ...
        hard_terminate = self.constraint_mode == "hard" and (cap_violated or conflict_violated)
        done = hard_terminate or self._step >= self.M
        total_viol = self.capacity_violations + self.conflict_violations
        terminal_bonus = 0.0
        if hard_terminate:
            terminal_bonus = -float(self.M - self._step)
        elif done:
            terminal_bonus = self.ar if total_viol == 0 else -self.ar

        if self.constraint_mode == "lagrange" and self.lagrange_state is not None and done:
            self.lagrange_state.update(
                cap_rate=self.capacity_violations / self._step,
                conflict_rate=self.conflict_violations / self._step,
            )
