"""
Regenerate config_ecu_gt_svc.yaml with ALL scenarios guaranteed ILP-feasible.
Same generator as scenarios/lt/ilp/config/generate_config.py, just N_ECUS/N_SVCS
sized for the "gt" scenario (ECU count > SVC count).
"""
...
N_ECUS = 15   # ECU count > SVC count (ecu_gt_svc)
N_SVCS = 10
...
def generate_feasible(scenario_id: int) -> dict:
    ...
    while True:
        ...
        # A conflict set draws from the N_SVCS services, so its size can't
        # exceed N_SVCS even when N_ECUS > N_SVCS (the gt case).
        max_set_size = min(N_ECUS, N_SVCS)
        conflict_sets = [
            sorted(random.sample(range(N_SVCS), random.randint(2, max_set_size)))
...
def _build_config(scenario_id, ecu_capacity, svc_requirement, conflict_sets) -> dict:
    ...
    print(f"  Scenario {scenario_id:>4d} [feasible]: {N_ECUS} ECUs, {N_SVCS} SVCs")
    return {
...
if __name__ == "__main__":
    ...
    write_config(scenarios, "config_ecu_gt_svc.yaml")
