...
def train_lagrange_bc(device: str, n_envs: int = 1):
    ...
    # gt's env does not sort services descending in reset() (unlike
    # eq/lt's LagrangeEnv) — sorted_desc=False keeps expert actions
    # aligned with the env's actual (unsorted) presentation order.
    obs_arr, _mask_arr, act_arr = build_bc_dataset(C.TRAIN_SCENARIOS, LagrangeEnv, use_mask=False, sorted_desc=False)
