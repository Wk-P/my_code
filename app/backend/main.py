...
def _run_dirs(algo_dir: Path) -> list[Path]:
    ...
    return sorted(runs, key=lambda d: d.name)  # timestamp dirs sort lexicographically = chronologically
...
PROGRESS_RE = re.compile(
    r"\[train\]\s+step=([\d,]+)/([\d,]+)\s+\(\s*([\d.]+)%\)\s+\|\s+eps=(\d+)\s+\|\s+steps/s=([\d,]+)"
)
...
def _tail_log_progress(path_str: str) -> dict:
    ...
    completed_algos, current_algo = [], None
    for line in lines:
        ...
        if event == "starting":
            current_algo = algo
        elif event == "finished" and algo not in completed_algos:
    ...
    for line in reversed(lines):
        ...
        if m:
            latest_progress = {
                ...
                "total_steps":   int(m.group(2).replace(",", "")),
                "pct":           float(m.group(3)),
                "episodes":      int(m.group(4)),
                "steps_per_sec": int(m.group(5).replace(",", "")),
            }
            break
    ...
    overall_pct = round(min(100.0, (done + within_current) / total * 100), 1)
...
def get_progress():
    ...
    for scenario in SCENARIOS:
        ...
        if proc:
            _, algo = _match_scenario_algo(proc["cmd"])
            entry.update({
                "current_algo":    algo,
                "pid":             proc["pid"],
                "core":            proc["psr"],
        ...
        entry.update(_tail_log_progress(log_sources.get(scenario, "")))
