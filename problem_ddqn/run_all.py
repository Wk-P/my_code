...
def plot_comparison(ilp_ar, rand_res, ddqn_res, ddqn_train_viol_mean, ddqn_train_viol_std, outdir, scenario_name):
    ...
    ax2.set_ylim(0, 1.1)
    ax2.set_ylabel("Violation Rate", fontsize=11)
    ax2.set_title("Violation Rate (train-end for DDQN)", fontsize=11)
...
def main():
    ...
    model, cb = train_ddqn(ecus, services)
    ...
    ddqn_train_v = np.mean(cb.episode_violated[-50:]) if len(cb.episode_violated) >= 50 else np.mean(cb.episode_violated)
    ddqn_train_v_std = np.std(cb.episode_violated[-50:]) if len(cb.episode_violated) >= 50 else np.std(cb.episode_violated)
