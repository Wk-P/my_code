"""
plot_aggregate_combined_compassed.py — 3×4 combined figure, compassed output.
...
OUT_DIR  = ROOT / "logs/multi_seed_results/figures/compassed"
...
COL_TITLES = [
    ...
    "Sum Placed Services",
    "Avg Capacity Violations",
    "Avg Privacy Violations",
]
...
def add_bar_labels(ax, bars, fmt=".2f"):
    ...
    for bar in bars:
        ...
        ax.text(bar.get_x() + bar.get_width() / 2,
                ...
                fontsize=6, fontweight="bold", color=RL_COLOR)
...
def draw_row(axes_row, rl_ar, rl_placed, rl_cap, rl_conflict, ilp_ar,
    ...
    for bar, true_val in zip(bars, rl_ar):
        ...
        label   = f"↑{true_val:.2f}" if clipped else f"{true_val:.2f}"
    ...
    ax.set_xlabel("Safe RL Models", fontsize=8)
    ...
    ax = axes_row[3]
    if show_col_titles:
        ax.set_title(COL_TITLES[3], fontsize=10, fontweight="bold", pad=6)
